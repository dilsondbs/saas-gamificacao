...
class RegistrationController extends Controller
{
    ...
    public function showStep2(Request $request)
    {
        ...
        return Inertia::render('Central/Registration/Step2', [
            'step1Data' => $step1Data,
    ...
    {
        \Log::info('🚀 Step2 POST request received', $request->all());
        ...
        \Log::info('✅ Step2 validation passed', $validated);
    ...
    public function complete(Request $request)
    {
        ...
        \Log::info('🔍 Dados da sessão', [
            'step1_exists' => !empty($step1Data),
            'step2_exists' => !empty($step2Data),
            'step1_keys' => $step1Data ? array_keys($step1Data) : [],
            'step2_keys' => $step2Data ? array_keys($step2Data) : []
        ]);

        if (!$step1Data || !$step2Data) {
            return back()->withErrors(['error' => 'Dados de registro não encontrados. Reinicie o processo.']);
        }

        \Log::info('🚀 INICIANDO CRIAÇÃO SÍNCRONA DE TENANT', [
            'tenant_name' => $step2Data['tenant_name'],
            'tenant_slug' => $step2Data['tenant_slug'],
            'plan' => $step1Data['plan']
        ]);
        ...
        try {
            ...
            $result = $this->createTenantSync($step1Data, $step2Data);

            // Clear registration session data
            session()->forget(['registration_step1', 'registration_step2']);

            \Log::info('🎉 TENANT CRIADO COM SUCESSO (SÍNCRONO)', $result);

            // Return success page with tenant info
            return Inertia::render('Central/Registration/Success', [
                'tenantInfo' => $result
            ]);

        } catch (\Exception $e) {
            \Log::error('💥 ERRO NA CRIAÇÃO SÍNCRONA DE TENANT', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);

            return back()->withErrors([
                'error' => 'Erro ao criar plataforma: ' . $e->getMessage()
            ]);
        }
    }

    /**
     * Create tenant synchronously (simplified)
     */
    private function createTenantSync($step1Data, $step2Data)
    {
        \Log::info('📝 Validando dados de entrada');

        // 1. Validate data
        if (empty($step1Data['company_name']) || empty($step2Data['tenant_slug'])) {
            throw new \Exception('Dados obrigatórios não fornecidos');
        }

        \Log::info('🗑️ Executando limpeza preventiva');

        // 2. Emergency cleanup - remove any conflicting data FIRST
        try {
            // Hard delete from database to avoid soft delete issues
            $deletedCount = \DB::table('tenants')->where('slug', $step2Data['tenant_slug'])->delete();
            if ($deletedCount > 0) {
                \Log::info('🗑️ Removidos tenants conflitantes', ['count' => $deletedCount, 'slug' => $step2Data['tenant_slug']]);
            }

            // Also clean domains table
            $deletedDomains = \DB::table('domains')->where('domain', 'like', '%' . $step2Data['tenant_slug'] . '%')->delete();
            if ($deletedDomains > 0) {
                \Log::info('🗑️ Removidos domínios conflitantes', ['count' => $deletedDomains]);
            }

            // Drop databases with potential conflicts
            $databases = ["tenant{$step2Data['tenant_slug']}", $step2Data['tenant_slug']];
            foreach ($databases as $dbName) {
                try {
                    \DB::statement("DROP DATABASE IF EXISTS `{$dbName}`");
                    \Log::info('🗑️ Database dropped', ['database' => $dbName]);
                } catch (\Exception $e) {
                    \Log::warning('Database drop warning: ' . $e->getMessage());
                }
            }
        } catch (\Exception $e) {
            \Log::warning('Limpeza preventiva warning: ' . $e->getMessage());
        }

        // 3. Final validation after cleanup
        if (Tenant::where('slug', $step2Data['tenant_slug'])->exists()) {
            \Log::error('🚫 Tenant ainda existe após limpeza!');
            throw new \Exception('Falha na limpeza: tenant ainda existe após remoção');
        }

        \Log::info('🏗️ Criando tenant na central');

        // 4. Create tenant in central database
        $maxUsers = match($step1Data['plan']) {
            'teste' => 5,
            'basic' => 50,
            'premium' => 200,
            'enterprise' => 1000,
        ...
        \Log::info('🌐 Criando domínio para tenant');
        ...
        \Log::info('🗄️ Sistema multi-tenant com banco único - não precisa de migrações separadas');

        // Migrações já aplicadas no banco único - pular

        \Log::info('👤 Criando usuário administrador');

        // 7. Create admin user with tenant_id
        try {
        ...
        return [
            ...
            'plan' => $tenant->plan,
            'access_url' => "http://{$domainName}:8000",
            'login_instructions' => [
                '1. 🌐 Acesse: http://' . $domainName . ':8000',
                '2. 📧 Email: ' . $step1Data['company_email'],
                '3. 🔑 Senha Temporária: temporary123',
                '4. 🔐 Você será solicitado a alterar sua senha',
                '5. ⚙️ Configure sua plataforma',
                '6. 👥 Adicione usuários e conteúdo'
            ]
        ];
    }

    /**
     * Create tenant asynchronously (DEPRECATED - kept for reference)
     */
    private function createTenantAsync($creationId, $step1Data, $step2Data)
    {
        ...
        $updateStatus = function($status, $progress, $step, $message) use ($creationId) {
            ...
            $tenant->run(function () use ($step1Data, $step2Data, $tenantSlug, &$adminCreated) {
                ...
                // IMPORTANTE: Senha temporária padrão para TODOS os tenants
                ...
                \Log::info('🔑 Admin criado com senha temporária PADRÃO', [
                    'email' => $adminEmail,
                    'temporary_password' => $temporaryPassword,
                    'must_change' => true,
                    'IMPORTANTE' => 'Senha padrão temporary123 para todos os tenants'
                ]);

                // Test credentials with temporary password
            ...
            // REMOVIDO: Seeder automático para dados reais
            // Para tenants reais, criamos apenas o usuário admin correto
            // Seeders ficam opcionais apenas para desenvolvimento/testes
            \Log::info('📋 Saltando seeder automático para tenant real');
